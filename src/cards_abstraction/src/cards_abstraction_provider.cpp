    ...
    uint32_t iso_number, Round r, std::vector<Card>& public_cards_out)
{
  ...
  switch(r) {
    case PREFLOP:
      // TODO(Levon): do something over here.
      break;
    case FLOP:
...
void CardsAbstractionProvider::get_canonical_cards_combination(
  ...
  // TODO(levon): You're doing something wrong over here. 52^52 is a super big number. 
  // God knows how this still works.
  ...
  uint32_t public_iso_number = uint32_t((iso_number-holeIsoNumber) / pow(52, 52));
...
uint32_t CardsAbstractionProvider::get_iso_public_cards_number(Round r) {
  switch(r) {
    case PREFLOP:
      ...
      // TODO(levon): it can, please change.
    case FLOP :
